  ...
  try {
    ...
    try {
      ...
      if (h && h.ok) {
        ...
        /* 📉 وعمق التاريخ: بياناتٌ محدّثة اليوم لكنها عشرات الجلسات لا
           مئات تُنتج تحليلاً يبدو تامّاً وهو على جذعٍ مقطوع. */
        if (h.shallowN > 0) {
          health += '\n📉 تاريخ قصير (<240 جلسة): ' + h.shallowN + ' سهماً'
            + (h.shallow && h.shallow.length
                ? '\nأقصرها: ' + h.shallow.map(w => w.sym + ' (' + w.bars + ')').join(' · ') : '')
            + '\n↩️ أعِد «جلب/تحديث البيانات التاريخية (5 سنوات)».';
        }
      }
    } catch (_) {}
